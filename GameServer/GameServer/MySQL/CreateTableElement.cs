...
public class CreateTableElement
{
    ...
    private string Cmd
    {
        get { return GetHeadCmd() + GetElementCmd() + GetTailCmd(); }
    }
    ...
    private string GetElementCmd()
    {
        ...
        foreach (Element e in elementList)
        {
            ...
            element += ",";
            if (e.isPrimarkKey) primark += string.Format(" PRIMARY KEY ( `{0}` )", e.keyName);
        }
    ...
    private string GetTailCmd()
    {
        return " PRIMARY KEY ( `RowId` ))ENGINE=InnoDB" + MySqlTemplate.GetEncodingCmd(encoding);
    }
}
